...
open at the upper end; e.g. [_FMIN_,_FMAX_).  That is, a channel is\n\
...
main( int argc, char **argv )
{
  ...
  /* Get bin range. */
  if ( mode == 'f' ) {
    nmin = (int)floor( ( fmin - head.dim2_start - head.frequency_offset_DC )
		       *head.dims[1]/head.dim2_span );
    nmax = (int)floor( ( fmax - head.dim2_start - head.frequency_offset_DC )
		       *head.dims[1]/head.dim2_span );
  } else if ( mode != 'n' ) {
    nmin = 0;
    nmax = head.dims[1];
