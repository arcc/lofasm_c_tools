...
// The units of input frequencies are in Hz
// The out put time unit is seconds.
{
    double timeDelay;
...
int sum_one_dm(int start_sum_idx_last, int end_sum_idx_last,
                int start_sum_idx_curr, int end_sum_idx_curr,
                int num_dedsps_time_bin,
                std::vector<double> & channel_data,
                std::vector<double> & result_last,
                std::vector<double> & result_curr)
{
  // This is a wrapper function of sum_channel. This function uses the same
  // API that is used in the sum_next_dm(). This allows it being called from one
  // unified API
  int res;
  res = sum_channel(num_dedsps_time_bin, start_sum_idx_curr, end_sum_idx_curr,
                    channel_data, result_curr);
  return 0;
}
...
// The DM informaiton is provided by the start and end summation index.
{
  ...
  for (i = 0; i < num_dedsps_time_bin; i++ ){
...
int no_sum(int start_sum_idx_last, int end_sum_idx_last,
                int start_sum_idx_curr, int end_sum_idx_curr,
                int num_dedsps_time_bin,
                std::vector<double> & channel_data,
                std::vector<double> & result_last,
